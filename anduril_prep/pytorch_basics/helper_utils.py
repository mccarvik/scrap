...
def plot_results(model, distances, times, filename='delivery_times_plot.png'):
    """
...
def plot_nonlinear_comparison(model, new_distances, new_times, filename='nonlinear_comparison_plot.png'):
    """
...
def plot_data(distances, times, normalize=False, filename='data_plot.png'):
    """
...
def plot_final_fit(model, distances, times, distances_norm, times_std, times_mean, filename='final_fit_plot.png'):
    """
...
def plot_training_progress(epoch, loss, model, distances_norm, times_norm, filename=None):
    """
    ...
    Args:
        ...
        filename: Optional. If provided, saves the plot instead of showing it.
    """
    ...
    # If filename is provided, save the plot; otherwise show it
    if filename:
        plt.savefig(filename, dpi=300, bbox_inches='tight')
        plt.close()
        print(f"Saved plot to: {filename}")
