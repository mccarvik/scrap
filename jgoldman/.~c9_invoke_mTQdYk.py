...
class Position():
    ...
    def grab_history(self, start, end=dt.datetime.today()):
        ...
        # hist = pd.read_csv('https://www.alphavantage.co/query?function=TIME_SERIES_DAILY_ADJUSTED&symbol={0}&apikey={1}&datatype=csv'.format(self.sym, API_KEY))
...
def run(pos, start_dt, end_dt=dt.datetime.today()):
    ...
    # spy_hist = pd.read_csv('https://www.alphavantage.co/query?function=TIME_SERIES_DAILY_ADJUSTED&symbol={0}&apikey={1}&datatype=csv'.format('SPY', API_KEY))
...
def timeline_chart(pos, port_navs, spy_hist):
    # Will be returning simple return not total return, could add in the dividends to the NAV to get total
    ...
    plt.plot(port_navs['adjusted_close'] / port_navs['adjusted_close'].ix[0] * 100, c='orange', label='port')
